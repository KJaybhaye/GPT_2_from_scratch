...
total_batch_size = 512 * 16  # 2**19, ~0.5M, in number of tokens
B = 4
T = 512
...
data_dir = "shakspear.txt"
train_loader = DistributedDataloader(data_dir, tokenizer, B, T, process_rank=ddp_rank, num_processes=ddp_world_size)
val_loader = DistributedDataloader(data_dir, tokenizer, B, T, process_rank=ddp_rank, num_processes=ddp_world_size)
...
model = GPT(GPTConfig(vocab_size=50304, n_layer=12, n_embd=768, n_head=12, block_size=512))
model.to(device)
if ddp:
